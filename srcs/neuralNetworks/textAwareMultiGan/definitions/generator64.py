        ...
        for m in self.modules():
            if isinstance(m, nn.Conv2d) or isinstance(m, nn.ConvTranspose2d):
                if m in self.block1.modules():
                    init.normal_(m.weight.data, mean=0.0, std=0.12)  # Inizializza i pesi per il blocco 1
                elif m in self.block2.modules():
                    init.normal_(m.weight.data, mean=0.0, std=0.03)  # Inizializza i pesi per il blocco 2
